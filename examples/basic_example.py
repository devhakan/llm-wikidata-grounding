...
This example demonstrates the complete hybrid fact-checking pipeline:
1. Vector Search on Wikidata for relevant entities
2. Retrieve statements (claims) for those entities
3. Rerank statements by relevance (Cross-Encoder)
4. Classify using Ollama LLM to determine verdict
...
_VERDICT_SYMBOLS = {"SUPPORTED": "✓", "REFUTED": "✗", "NOT_ENOUGH_INFO": "?"}
...
def main():
    ...
    checker = HybridFactChecker(verbose=True)
    ...
    passed = sum(1 for r, e in results if r.verdict == e)
    print(f"\nPassed: {passed}/{len(results)}")

    for result, expected in results:
        match = "✓" if result.verdict == expected else "✗"
        print(f"  {match} {result.claim[:50]}... → {result.verdict}")
